<?php

namespace Board\PluginShelf\Mcp;

use Board\PluginShelf\Events\ShelfTreeUpdated;
use Board\PluginShelf\Mcp\Concerns\ResolvesShelfNodes;
use Board\PluginShelf\Support\ShelfActivity;
use Illuminate\Contracts\JsonSchema\JsonSchema;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Attributes\Description;
use Laravel\Mcp\Server\Tool;

#[Description('Move a folder, note or file of a Shelf board to the trash (soft delete). Its whole branch disappears from the tree but is restorable; a scheduled purge removes trashed items after 30 days.')]
class ShelfTrashNodeTool extends Tool
{
    use ResolvesShelfNodes;

    public function handle(Request $request): Response
    {
        $request->validate(['node_id' => 'required|string']);

        $node = $this->activeNode((string) $request->get('node_id'));

        if ($node === null) {
            return Response::error('Node not found or access denied.');
        }

        if (! $this->userCanWrite($node->board)) {
            return Response::error('You have a read-only role on this board.');
        }

        $node->update(['archived_at' => now()]);

        ShelfActivity::log($node->board, 'shelf.node_trashed', $node, ['via' => 'mcp']);
        broadcast(new ShelfTreeUpdated($node->board_id));

        return Response::json(['id' => $node->public_id, 'trashed' => true]);
    }

    /**
     * @return array<string, JsonSchema>
     */
    public function schema(JsonSchema $schema): array
    {
        return [
            'node_id' => $schema->string()->description('Public id (ULID) of the node to trash.')->required(),
        ];
    }
}
